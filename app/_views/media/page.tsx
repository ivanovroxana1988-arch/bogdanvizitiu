...
export default function Media({searchParams}:{searchParams?:{lang?:string}}){
  ...
  return <>
    ...
    <section className="shell content-grid"><EditorialImage asset="speaking" kind="event" locale={locale}/><div className="prose"><p>{locale==='ro'?'Sursele externe sunt folosite ca dovadă și destinație. Fotografiile sau artwork-urile terților nu sunt copiate pe site fără permisiune.':'External sources are used as evidence and destinations. Third-party photography or artwork is not copied onto the site without permission.'}</p></div></section>
    <section className="programs shell">{appearances.map(item=><article className="program-row" key={item.id}><h3>{item.title}</h3><p>{item.type}{item.published_at?` · ${item.published_at}`:''}</p><ArrowLink href={item.external_url!}>{locale==='ro'?'Vezi sursa':'View source'}</ArrowLink></article>)}</section>
