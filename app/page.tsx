...
export default function Home({searchParams}:{searchParams?:{lang?:string}}){
  ...
  return <>
    ...
    <section className="point-of-view"><div className="shell point-grid"><EditorialImage asset="candid" kind="portrait"/><div><h2>{copy.pointTitle}</h2><p>{copy.pointText}</p><ArrowLink href="/about">{copy.secondaryCta}</ArrowLink></div></div></section>

    <section className="areas shell"><div className="section-head"><div><Eyebrow>{copy.areasEyebrow}</Eyebrow><h2 className="section-title">{copy.areasTitle}</h2></div></div><div className="area-list">{areas.map((area,i)=><article className="area-row" key={area.title}><span>0{i+1}</span><h3>{area.title}</h3><p>{area.description}</p></article>)}</div></section>
