...
export default function Insights({searchParams}:{searchParams?:{lang?:string}}){
  ...
  return <>
    ...
    <section className="shell insights"><div className="insight-grid">
      <article className="feature-insight"><ConceptImage asset="architectureStair" kind="insight"/><div className="story-meta"><span className="category">{insights[0].category}</span><span>{insights[0].readTime}</span></div><h3>{insights[0].title}</h3><p>{insights[0].excerpt}</p><ArrowLink href={`/insights/${insights[0].slug}`}>{copy.readArticle}</ArrowLink></article>
