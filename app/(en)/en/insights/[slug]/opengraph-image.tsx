...
import { toRomanianInsightSlug } from '@/lib/routes'
...
export default function OpenGraphImage({ params }: { params: { slug: string } }) {
  const internalSlug = toRomanianInsightSlug(params.slug)
  const insight = getPublishedInsights('en').find((item) => item.slug === internalSlug)
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        >
          <span>Bogdan Vizitiu</span>
          <span>bogdanvizitiu.com</span>
        </div>
      </div>
    ),
    size,
