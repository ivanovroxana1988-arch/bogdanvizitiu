...
type ConceptImageProps = {
  asset: ConceptKey
  kind?: 'wide' | 'editorial' | 'insight'
  className?: string
  locale?: Locale
}
...
const hasFirstPartyRights = (item: { status: string; rights_status: string }) =>
  item.status === 'approved' &&
  (item.rights_status === 'owned' ||
    item.rights_status === 'permission' ||
    item.rights_status === 'approved-for-site-by-user')
const hasConceptRights = (item: { status: string; rights_status: string }) =>
  item.status === 'approved' &&
...
const englishConceptAlt: Record<ConceptKey, string> = {
  ...
  emotionsLearningEditorial:
    'Editorial scene about the connection between emotional state, attention and learning',
...
export function EditorialImage({
  ...
  return (
    <figure
      ...
      aria-label={approved ? alt : undefined}
    >
      {approved ? (
        <Image
          src={item.src}
          ...
          fill
          priority={asset === 'hero'}
          quality={asset === 'hero' ? 90 : 75}
          sizes={
            asset === 'hero'
              ? '(max-width: 900px) 100vw, (max-width: 1424px) 55vw, 748px'
              : '(max-width: 900px) 100vw, 50vw'
          }
        />
      ) : (
        <div className="image-placeholder" aria-hidden="true">
          <span>BGV / IMAGE {index[asset]}</span>
          <i />
          <span>{kind === 'portrait' ? 'PORTRAIT 4:5' : 'EDITORIAL 3:2'}</span>
...
export function ConceptImage({
  ...
  return (
    <figure className={`${styles.conceptImage} ${styles[kind]} ${className}`}>
      {approved ? (
        <img
          src={item.src}
          alt={alt}
...
export const Portrait = ({
  ...
  <EditorialImage
    asset={event ? 'speaking' : 'hero'}
    kind={event ? 'event' : 'portrait'}
    locale={locale}
  />
)
