...
function requireString(value: unknown, collection: string, path: string) {
  if (typeof value !== 'string' || value.trim() === '')
    fail(collection, path, 'expected non-empty string')
  return value
}
...
function requireStringArray(value: unknown, collection: string, path: string) {
  if (!Array.isArray(value) || value.some((item) => typeof item !== 'string'))
    fail(collection, path, 'expected string[]')
...
function isLocalizedStringArray(value: unknown): value is Localized<string[]> {
  return (
    isRecord(value) &&
    Array.isArray(value.ro) &&
    value.ro.every((item) => typeof item === 'string') &&
...
const allowedRights = new Set<RightsStatus>([
  'owned',
  'permission',
  'approved-for-site-by-user',
  'licensed',
...
export function isPublishableCourseProduct(product: ProductRecord): product is CourseProductRecord {
  ...
  if (!Array.isArray(product.dates) || product.dates.some((item) => typeof item !== 'string'))
    return false
  if (
    !(
      product.price === null ||
...
function validateAssetMap(input: unknown, collection: string, path: string) {
  ...
  for (const [key, raw] of Object.entries(root)) {
    ...
    const rights = requireString(
      item.rights_status,
      collection,
      `${path}.${key}.rights_status`,
    ) as RightsStatus
    if (!allowedRights.has(rights))
      fail(collection, `${path}.${key}.rights_status`, `unsupported rights status ${rights}`)
...
export function validateMediaCollection(input: unknown, collection = 'media'): MediaCollection {
  ...
  const appearances = root.appearances.map((raw, index) => {
    ...
    const rights = requireString(
      item.rights_status,
      collection,
      `appearances[${index}].rights_status`,
    ) as RightsStatus
    if (!allowedRights.has(rights))
      fail(collection, `appearances[${index}].rights_status`, `unsupported rights status ${rights}`)
    if (item.external_url !== undefined)
      requireString(item.external_url, collection, `appearances[${index}].external_url`)
  ...
  return {
    ...
    concepts,
    appearances,
    editorialNote: typeof root.editorialNote === 'string' ? root.editorialNote : undefined,
  }
}
...
export function validateTestimonials(
  ...
  const items = root.items.map((raw, index) => {
    ...
    if (item.role_company !== undefined)
      requireString(item.role_company, collection, `items[${index}].role_company`)
...
export function validateProfile(input: unknown, collection = 'profile'): ProfileRecord {
  ...
  if (!(root.biography === null || typeof root.biography === 'string'))
    fail(collection, 'biography', 'expected string|null')
...
export function canPublishAsset(asset: AssetRecord) {
  const rightsOk =
    ...
    asset.rights_status === 'permission' ||
    asset.rights_status === 'approved-for-site-by-user' ||
...
export function canPublishAppearance(item: AppearanceRecord) {
  const statusOk =
    item.status === 'verified' || item.status === 'approved' || item.status === 'published'
  const rightsOk =
    item.rights_status === 'embed-only' ||
    item.rights_status === 'permission' ||
