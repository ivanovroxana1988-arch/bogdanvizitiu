...
const insightSlugRoToEn: Record<string, string> = {
  'networkingul-nu-incepe-cu-schimbul-de-contacte':
    'networking-does-not-start-with-exchanging-contacts',
  'de-la-unde-sunt-la-ce-fac-mai-departe-modelul-lives':
    'from-where-i-am-to-what-i-do-next-lives-model',
  ...
  'de-ce-unele-conversatii-manageriale-schimba-lucrurile':
    'why-some-management-conversations-change-things',
  ...
  'coaching-sau-consultanta-de-ce-ai-nevoie-de-fapt':
    'coaching-or-consulting-what-do-you-actually-need',
}

const enToRo = Object.fromEntries(Object.entries(roToEn).map(([ro, en]) => [en, ro])) as Record<
  string,
  string
>
const programSlugEnToRo = Object.fromEntries(
  Object.entries(programSlugRoToEn).map(([ro, en]) => [en, ro]),
) as Record<string, string>
const insightSlugEnToRo = Object.fromEntries(
  Object.entries(insightSlugRoToEn).map(([ro, en]) => [en, ro]),
) as Record<string, string>

export function toEnglishProgramSlug(slug: string) {
  return programSlugRoToEn[slug] ?? slug
}
export function toRomanianProgramSlug(slug: string) {
  return programSlugEnToRo[slug] ?? slug
}
export function toEnglishInsightSlug(slug: string) {
  return insightSlugRoToEn[slug] ?? slug
}
export function toRomanianInsightSlug(slug: string) {
  return insightSlugEnToRo[slug] ?? slug
}
...
function toEnglish(pathname: string) {
  ...
  if (pathname.startsWith('/cursuri/'))
    return `/en/programs/${toEnglishProgramSlug(pathname.slice('/cursuri/'.length))}`
  if (pathname.startsWith('/insights/'))
    return `/en/insights/${toEnglishInsightSlug(pathname.slice('/insights/'.length))}`
...
function toRomanian(pathname: string) {
  ...
  if (pathname.startsWith('/en/programs/'))
    return `/cursuri/${toRomanianProgramSlug(pathname.slice('/en/programs/'.length))}`
  if (pathname.startsWith('/en/insights/'))
    return `/insights/${toRomanianInsightSlug(pathname.slice('/en/insights/'.length))}`
